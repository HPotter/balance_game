...
public class BalanceView extends View {
	...
	private List<WeightedObject> mObjectsOnLeft, mObjectsOnRight, mAvaliableObjects;
	...
	public BalanceView(Context context, TaskData taskData, int i){
		...
		loadAndStartTask(context,
				...
				taskData.getBalanceData().get(i).getObjectsOnRight(),
				taskData.getBalanceData().get(i).getAvaliableObjects(), 
				taskData.getBalanceData().get(i).isInteractive(),
				taskData.getBalanceData().get(i).isFixed(),
	...
	public void loadAndStartTask(Context context, List<WeightedObject> _objectsOnLeft,
...
//		super.onDraw(canvas);
	...
	public void drawOnCanvas(Canvas canvas){
		...
		if (!fixed){
			...
			if(mCurrentState != mPreviousState){
				...
				case EQUAL:
					...
					break;
				default:
					throw new IllegalArgumentException("Unhandled state of balance");
				}
				mPreviousState = mCurrentState;
				AnimationUpdateThread at = new AnimationUpdateThread(degreeClause, direction);
		...
		if (!mInitPlacementFinished){
			...
			mBeam.setX(mSupport.getX() + mSupport.getBitmap().getWidth()/2 - mBeam.getBitmap().getWidth()/2);
			mBeam.setY(mSupport.getY());
			Collections.sort(mObjectsOnLeft, new WeightedObject.HeightComparator());
			Collections.sort(mObjectsOnRight, new WeightedObject.HeightComparator());
			Collections.sort(mAvaliableObjects, new WeightedObject.HeightComparator());
			switch (mCurrentState){
			case EQUAL:
				mBeamBent = mBeam.copy();
				mLeftCup.setX(mBeam.getX() - mLeftCup.getBitmap().getWidth()/2 + 40);
				mLeftCup.setY(mSupport.getY() + mBeam.getBitmap().getHeight()/2 - mLeftCup.getBitmap().getHeight() + 5);
				mRightCup.setX(mBeam.getX() - mRightCup.getBitmap().getWidth()/2 + mBeam.getBitmap().getWidth() - 41);
				mRightCup.setY(mSupport.getY() + mBeam.getBitmap().getHeight()/2  - mRightCup.getBitmap().getHeight() + 5);
				mDegree = 0;
				mRotationAnimation.postTranslate(mBeamBent.getX(), mBeamBent.getY());
				break;
			case LEFT_IS_HEAVIER:
				mRotationAnimation.postRotate(-14, mBeam.getBitmap().getWidth()/2, mBeam.getBitmap().getHeight()/2);
				mBeamBent = mBeam.copy();
				mBeamBent.setX(mSupport.getX() + mSupport.getBitmap().getWidth()/2 - mBeamBent.getBitmap().getWidth()/2);
				mBeamBent.setY(mSupport.getY() - mBeamBent.getBitmap().getHeight()/2 + mBeam.getBitmap().getHeight()/2);
				mLeftCup.setX(mBeam.getX() - mLeftCup.getBitmap().getWidth()/2 + 41);
				mLeftCup.setY(mSupport.getY() + mBeam.getBitmap().getHeight()/2 - mLeftCup.getBitmap().getHeight() + 5 + mDisbalanceOffsetY);
				mRightCup.setX(mBeam.getX() + mBeam.getBitmap().getWidth() - mRightCup.getBitmap().getWidth()/2 - 41);
				mRightCup.setY(mSupport.getY() + mBeam.getBitmap().getHeight()/2  - mRightCup.getBitmap().getHeight() + 5 - mDisbalanceOffsetY);
				mRotationAnimation.postTranslate(mBeamBent.getX(), mBeamBent.getY());
				mDegree = -14;
				break;
			case RIGHT_IS_HEAVIER:
				...
				mLeftCup.setY(mSupport.getY() + mBeam.getBitmap().getHeight()/2 - mLeftCup.getBitmap().getHeight() + 5 - mDisbalanceOffsetY);
				mRightCup.setX(mBeam.getX() + mBeam.getBitmap().getWidth() - mRightCup.getBitmap().getWidth()/2 - 41);
				mRightCup.setY(mSupport.getY() + mBeam.getBitmap().getHeight()/2  - mRightCup.getBitmap().getHeight() + 1 + mDisbalanceOffsetY);
				mRotationAnimation.postTranslate(mBeamBent.getX(), mBeamBent.getY());
				mDegree = 14;
				break;
			}
		...
		canvas.drawBitmap(mLeftCup.getBitmap(),
				...
				mLeftCup.getY(),
				mAntiAliasingPaint);
		canvas.drawBitmap(mRightCup.getBitmap(),
				mRightCup.getX(),
				mRightCup.getY(),
				mAntiAliasingPaint);
		canvas.drawBitmap(mBeamBent.getBitmap(), mRotationAnimation, mAntiAliasingPaint);
		canvas.drawBitmap(mSupport.getBitmap(),
				mSupport.getX(),
				mSupport.getY(),
				mAntiAliasingPaint);
	...
	private class AnimationUpdateThread implements Runnable{
		...
		public void run() {
			...
			while(currentSignum == Math.signum(mDegree - degreeClause)){
				...
				mRotationAnimation.reset();
				mBeamBent.setX(mSupport.getX() + mSupport.getBitmap().getWidth()/2 - mBeam.getBitmap().getWidth()/2);
				mBeamBent.setY(mSupport.getY() - mBeamBent.getBitmap().getHeight()/2 + mBeam.getBitmap().getHeight()/2);
				mRotationAnimation.postRotate(mDegree, mBeam.getBitmap().getWidth()/2, 
						mBeam.getBitmap().getHeight()/2);
				mRotationAnimation.postTranslate(mBeamBent.getX(), mBeamBent.getY());
				for (WeightedObject wo : mObjectsOnRight){
					wo.setY(mRightCup.getY() + mRightCup.getBitmap().getHeight()*3/4 - wo.getBitmap().getHeight());
				}
				for (WeightedObject wo : mObjectsOnLeft){
					wo.setY(mLeftCup.getY() + mLeftCup.getBitmap().getHeight()*3/4 - wo.getBitmap().getHeight());
				}
				...
				if (eventY > BASE_HEIGHT/2){
					...
					for (int i = mObjectsOnLeft.size() - 1; i >= 0; --i){
						...
						if (mObjectsOnLeft.get(i).isTouched(eventX, eventY)){
							getParent().requestDisallowInterceptTouchEvent(true);
							mDraggedObject = (WeightedObject) mObjectsOnLeft.get(i).copy();
							mDraggedObjectIndex = i;
			...
			case(MotionEvent.ACTION_MOVE):
				if (mDragOngoing){
					if (eventX + mDraggedObject.getBitmap().getWidth()/2 <= BASE_WIDTH
						&& eventX - mDraggedObject.getBitmap().getWidth()/2>= 0){ 
					...
					if (eventY + mDraggedObject.getBitmap().getHeight()/2<= BASE_HEIGHT
							&& eventY - mDraggedObject.getBitmap().getHeight()/2>= 0){ 
